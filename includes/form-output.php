...
<style type="text/css">.leadhide{ display: none; }</style>
...
if ( $open_section == true ) :
	...
	if ( $field_type == 'fieldset' ) :
		...
		$output .= sprintf(
			'<fieldset class="sti-fieldset sti-fieldset-%1$d %2$s %3$s" name="'.$field_key.'" id="item-%4$s">%5$s<ul class="sti-section sti-section-%1$d">',
			$count,
		...
		if($field_type=='radio' || $field_type=='checkbox'){
					...
					$output .= sprintf(
					'<div nonplace class="sti-item '.$addhide.' sti-item-%1$s %2$s %3$s" id="item-%4$s" style="'.$hide.'"><label' . $for . ' class="sti-desc">%5$s %6$s</label>',
					$field_type,
					$columns_choice,
					$layout,
					$id_attr,
					...
					$output .= sprintf(
					'<div placeholder class="sti-item '.$addhide.' sti-item-%1$s %2$s %3$s" id="item-%4$s" style="'.$hide.'">',
					$field_type,
					$columns_choice,
					$layout,
					$id_attr,
		...
		elseif ( in_array( $field_type, array( 'verification', 'secret' ) ) ) :
			if ( $field_type == 'verification' ) :
				$verification .= sprintf(
					'<fieldset class="sti-fieldset sti-fieldset-%1$d %2$s %3$s" id="item-%4$s" style="display:block"><div class="sti-legend"><h3>%5$s</h3></div><ul class="sti-section sti-section-%1$d">',
					$count,
					esc_attr( $field->field_key ),
					$css,
					$id_attr,
			...
			switch ( $field_type ) {  
                ...
                if($field_key=='SelectedLeadSource'){
					...
					$output .='<input type="hidden" name="SelectedLeadSource" value="2890">';
					}
					else{
			...
			// Loop through each option and output
				...
				$address .= sprintf(
					'<div class="sti-%3$s">' . $placement_top . '<select name="sti-%1$d[%4$s]" class="sti-select %7$s %8$s" id="%2$s-%4$s">%6$s</select><div id="val_'.$field_key.$form_id.'" class="sti-error"></div>' . $placement_bottom . '</div>',
					$field_id,
					$id_attr,
